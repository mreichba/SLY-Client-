...
export default class InitialStats extends React.Component {
  ...
  componentDidMount() {
    StatsService.getInitialQuizStats(TokenService.getAuthToken()).then(
      response => {
        ...
        <div>
          <ul className='initialUL'>
            ...
            <li>
              <h5>Location</h5>
              <p className='ans'>
                {this.state.userData.userData.location.charAt(0).toUpperCase() +
                  this.state.userData.userData.location.substring(1)}
              </p>
              <p className='desc'>
                {Math.floor(
